...
const ReportTable = ({ lostReports, returnedReports, onDelete, onConfirm }) => {
  ...
  try {
    ...
    // Hapus dari koleksi asli
    await deleteDoc(docRef);

    // Simpan ke histori dengan data yang valid
    const historyRef = doc(collection(db, 'history_items'));
    await setDoc(historyRef, {
      namaBarang: report.namaBarang || 'Tanpa Nama',
      kategori: report.kategori || 'Tidak Diketahui',
      foto: report.foto || '',
      type: report.type,
      takenAt: new Date(),
      status: 'taken',
    ...
    console.error('Error in handleChecklist:', error);
